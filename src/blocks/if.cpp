...
void IfStatement::codegen(CompilerContext &context) const {
  ...
  auto if_block = llvm::BasicBlock::Create(context.llvm_context, "if_block",
                                           currentFunction);
  auto else_block = llvm::BasicBlock::Create(context.llvm_context, "else_block",
                                             currentFunction);
  auto cont_block = llvm::BasicBlock::Create(context.llvm_context,
                                             "ifcont_block", currentFunction);
