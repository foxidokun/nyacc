...
TypedValue BinopExpression::codegen(CompilerContext &context) const {
  ...
  switch (type_) {
    ...
    DISPATCH_BINOP(LT, CreateFCmpOLT, CreateICmpSLT, CreateICmpULT,
                   cmp_result_type)
    DISPATCH_BINOP(LE, CreateFCmpOLE, CreateICmpSLE, CreateICmpULE,
                   cmp_result_type)
    DISPATCH_BINOP(GT, CreateFCmpOGT, CreateICmpSGT, CreateICmpUGT,
                   cmp_result_type)
    DISPATCH_BINOP(GE, CreateFCmpOGE, CreateICmpSGE, CreateICmpUGE,
                   cmp_result_type)
    DISPATCH_BINOP(EQ, CreateFCmpOEQ, CreateICmpEQ, CreateICmpEQ,
                   cmp_result_type)
    DISPATCH_BINOP(NE, CreateFCmpONE, CreateICmpNE, CreateICmpNE,
                   cmp_result_type)
  default:
...
static ValType get_result_type(ValType lhs, ValType rhs) {
  if (lhs.kind == ValType::Kind::Void || lhs.kind == ValType::Kind::String ||
      rhs.kind == ValType::Kind::Void || rhs.kind == ValType::Kind::String) {
  ...
  // Only one float
  if (lhs.kind == ValType::Kind::Float) {
    return lhs;
  }
  if (rhs.kind == ValType::Kind::Float) {
    return rhs;
  }
