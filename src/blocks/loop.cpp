...
void LoopStatement::codegen(CompilerContext &context) const {
  ...
  auto check_block = llvm::BasicBlock::Create(context.llvm_context,
                                              "check_block", currentFunction);
  auto loop_block = llvm::BasicBlock::Create(context.llvm_context, "loop_block",
                                             currentFunction);
  auto cont_block = llvm::BasicBlock::Create(
      context.llvm_context, "afterloop_block", currentFunction);
